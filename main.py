...
image = modal.Image.debian_slim("3.12").pip_install_from_requirements(
    "requirements.txt"
)
# Create Modal app
...
webhook_secret = modal.Secret.from_name(
    "executive-agent", required_keys=["ZAPIER_WEBHOOK_URL", "ANTHROPIC_API_KEY"]
)


@app.cls(
    volumes={
        VOLUME_DIR: modal.Volume.from_name("executiveagent", create_if_missing=True)
    },
...
class AgentAPI:
    ...
    def query(self, data: QueryRequestT) -> Dict[str, Any]:
        ...
        agent = ExecutiveAgent(
            enable_extended_thinking=True, db_path=os.path.join(VOLUME_DIR, "tasks.db")
        )
        """
        ...
        if steps:
            ...
            return {"response": message, "zapier_sent": zapier_success}
